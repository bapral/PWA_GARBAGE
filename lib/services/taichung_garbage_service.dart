...
/// 核心邏輯包括：
/// 1. 唯一識別碼映射：以「車牌號碼」作為動態位置與靜態班表（rid=68d1...）的連結紐帶。
/// 2. ISO T 格式解析：處理 "20240411T093000" 這種包含日期與 T 字元的時間字串。
/// 3. 週循環過濾：自動判定當前星期幾，並篩選出當天有收運的點位。
/// 4. 混合資料源：整合靜態收運點與即時座標 API，補全班表缺失的地理資訊。
/// 
/// [執行順序說明]
/// 1. `syncDataIfNeeded`：初始化時下載即時座標與詳細班表，並執行 Isolate 解析映射。
/// 2. `fetchTrucks`：抓取動態 API 並執行時間格式標準化轉換。
/// 3. `findTrucksByTime`：用於預測模式，根據資料庫映射關係推估車輛位置。
